...
public class AutoFarAzul extends NextFTCOpMode {
    {
        addComponents(
                new SubsystemComponent(Intake.INSTANCE, Lock.INSTANCE, Hood.INSTANCE, Indexer.INSTANCE),
    ...
    public static double goal = 2000;
    ...
    public void onInit() {
        ...
        Turret.snap1Angle    = 0.0;
        Turret.snap2Angle    = 45.0;
        Turret.snapTolerance = 3.0;
    }
    @Override
    ...
    private SequentialGroup intakeHp() {
        return new SequentialGroup(
                intake(),
                new FollowPath(AutoPathsFar.IntakeHp(PedroComponent.follower()),true, 0.7),
                new FollowPath(AutoPathsFar.shootHp(PedroComponent.follower())),
                new WaitUntil(Turret::isSnapBrakeActive),
                shootar(),
                intake()
        );
    }
    private SequentialGroup intakeHp2() {
        return new SequentialGroup(
                intake(),
                new FollowPath(AutoPathsFar.IntakeHp2(PedroComponent.follower()), true, 0.7),
                new FollowPath(AutoPathsFar.shootHp2(PedroComponent.follower())),
                new WaitUntil(Turret::isSnapBrakeActive),
                shootar(),
                intake()
        );
    ...
    private SequentialGroup fileiraPaths() {
        return new SequentialGroup(
                ...
                new Command() {
                    @Override
                    public void start() {
                        Turret.enableSnapMode(Turret.SnapPosition.SHOT1);
                    }
                    @Override
                    public boolean isDone() { return true; }
                },
                new WaitUntil(Turret::isSnapBrakeActive),
                new FollowPath(AutoPathsFar.intakeFileira(PedroComponent.follower())),
                new FollowPath(AutoPathsFar.shootintake1(PedroComponent.follower())),
                shootar(),
                new Command() {
                    @Override
                    public void start() {
                        Turret.enableSnapMode(Turret.SnapPosition.SHOT2);
                    }
                    @Override
                    public boolean isDone() { return true; }
                },
